...
class ConsoleReporter:
    ...
    def generate_cli_summary(self, n_top: int = 10):
        risky_files = self.results[
            self.results['risk_class'].isin(["Critical", "High", "Medium"])
        ].sort_values(
            by=['main_author_share', 'total_file_churn'], 
            ascending=[False, False]
        ).head(n_top)

        if risky_files.empty:
            self.console.print("[green]Análise concluída. Nenhum arquivo de alto risco ou crítico encontrado.[/green]")
            return
    ...
    def export_report(self, format: Literal["csv", "json"]):
        ...
        self.console.print(f"[bold green]Relatório exportado com sucesso para:[/bold green] {filename}")