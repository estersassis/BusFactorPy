...
def sample_dir_data():
    ...
    """
    return pd.DataFrame([
        {'file': 'tests/test_a.py',         'author': 'A', 'lines_added': 10, 'lines_deleted': 0},
        {'file': 'tests/unit/test_b.py',    'author': 'A', 'lines_added':  5, 'lines_deleted': 0},
        {'file': 'tests/unit/test_b.py',    'author': 'B', 'lines_added':  5, 'lines_deleted': 0},
        {'file': 'src/app/main.py',         'author': 'A', 'lines_added': 20, 'lines_deleted': 0},
        {'file': 'src/utils/helpers.py',    'author': 'C', 'lines_added': 30, 'lines_deleted': 0},
...
def test_group_by_directory_depth_1():
    ...
    calc = BusFactorCalculator(
        df, metric="churn", group_by="directory", depth=1
    )
    res = calc.calculate()
...
def test_group_by_directory_depth_2():
    ...
    df = sample_dir_data()
    calc = BusFactorCalculator(
        df, metric="churn", group_by="directory", depth=2
    )
    res = calc.calculate()
